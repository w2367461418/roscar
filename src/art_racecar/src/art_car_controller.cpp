...
L1Controller::L1Controller()
{
    ...
    pn.param("Vcmd", Vcmd, 1.0);  //车速
    pn.param("lfw", lfw, 0.13);
    pn.param("lrv", lrv, 10.0);

    //Controller parameter 控制参数
    ...
    Lfw = goalRadius = getL1Distance(Vcmd);   //目标半径
    foundForwardPt = false;
    goal_received = false;					  //目标接受
    goal_reached = false;					  //目标达到
...
void L1Controller::goalReachingCB(const ros::TimerEvent&)  //目标地点是否接收到
{

    if(goal_received)
    {
        double car2goal_dist = getCar2GoalDist();
        if(car2goal_dist < goalRadius)    //若车 距目标的距离 小于  目标的半径
        {
            goal_reached = true;
            goal_received = false;
...
void L1Controller::controlLoopCB(const ros::TimerEvent&) //发布的话题
{
    int count = 100;
    ...
    if(goal_received)    //接收到目的地
    {
        /*Estimate Steering Angle*/   //估计舵机角度
        double eta = getEta(carPose);    //预计到达时间
        if(foundForwardPt)
        {
            //舵机的转向角度      angle_gain猜想为kp
            cmd_vel.angular.z = baseAngle + getSteeringAngle(eta)*Angle_gain;
            /*Estimate Gas Input*/

            if(!goal_reached)           //若目标未达到
            {
