...
class L1Controller
{
    ...
    private:
        ...
        ros::Time current_time_1,last_time_1;
...
L1Controller::L1Controller()
{
    ...
    marker_pub = n_.advertise<visualization_msgs::Marker>("car_path", 10);              //发布给rviz的小车路径信息
    pub_ = n_.advertise<geometry_msgs::Twist>("car/cmd_vel", 1);                        //发布的额控制信息
...
void L1Controller::odomCB(const nav_msgs::Odometry::ConstPtr& odomMsg)
{
	...
	current_time_1 = ros::Time::now();

    double dt = current_time_1.toSec()-last_time_1.toSec();
    ROS_INFO("time cha = %lf\n",dt);
    odom = *odomMsg;    //当前小车位置
...
void L1Controller::goalCB(const geometry_msgs::PoseStamped::ConstPtr& goalMsg)
{
    try
    {
        ...
        tf_listener.transformPose("odom", ros::Time(0) , *goalMsg, "map" ,odom_goal);//进行坐标系的转换
        odom_goal_pos = odom_goal.pose.position;
        goal_received = true;
        goal_reached = false;

        /*Draw Goal on RVIZ*/    //在rviz上画出目标地点
        goal_circle.pose = odom_goal.pose;
        marker_pub.publish(goal_circle);
    }
    catch(tf::TransformException &ex)   //抓取意外   
    {
        ROS_ERROR("%s",ex.what());
...
{
    float car2wayPt_x = wayPt.x - carPose.position.x;    //规划好的目标位置 - 当前位置
    float car2wayPt_y = wayPt.y - carPose.position.y;
    double car_theta = getYawFromPose(carPose);          //得到当前位置的偏航角

    float car_car2wayPt_x = cos(car_theta)*car2wayPt_x + sin(car_theta)*car2wayPt_y;
    float car_car2wayPt_y = -sin(car_theta)*car2wayPt_x + cos(car_theta)*car2wayPt_y;

    if(car_car2wayPt_x >0) /*is Forward WayPt*/ //判断目标点的位置
        return true;
    else
        return false;
}
...
{
    double dx = wayPt.x - car_pos.x;               //规划好的目标位置 - 当前位置    
    double dy = wayPt.y - car_pos.y;
    double dist = sqrt(dx*dx + dy*dy);

    if(dist < Lfw)                                 //检测距离目标位置的距离   与 拟定距离 做比较
        return false;
    else if(dist >= Lfw)
        return true;
}
...
{
    geometry_msgs::Point carPose_pos = carPose.position;    //车的位置
    double carPose_yaw = getYawFromPose(carPose);           //车的偏航角
    geometry_msgs::Point forwardPt;                         //小车的位置
    geometry_msgs::Point odom_car2WayPtVec;                 //odom下小车的向量
    foundForwardPt = false;                                 /*猜测*/

    if(!goal_reached)        //若目标位置没有达到
    {
        for(int i =0; i< map_path.poses.size(); i++)    //遍历得到的规划的路径信息
        {
            ...
            try
            {
                tf_listener.transformPose("odom", ros::Time(0) , map_path_pose, "map" ,odom_path_pose);  //将odom框架下得到的路径信息 转化 为map坐标系下的路径信息
                geometry_msgs::Point odom_path_wayPt = odom_path_pose.pose.position;
                bool _isForwardWayPt = isForwardWayPt(odom_path_wayPt,carPose);    //传入的是  规划好的目标位置  与 当前小车位置

                if(_isForwardWayPt)                     //若目标点位置正常
                {
                    bool _isWayPtAwayFromLfwDist = isWayPtAwayFromLfwDist(odom_path_wayPt,carPose_pos); //传入的是  规划好的目标位置  与 当前小车位置
                    if(_isWayPtAwayFromLfwDist)                    //若大于拟定距离  则还未到达 
                    {
                        forwardPt = odom_path_wayPt;   //  传入 规划好的位置信息
                        foundForwardPt = true;
                        break;            //跳出for循环
                    }
                }
...
{
    geometry_msgs::Point car_pose = odom.pose.pose.position;  //里程计坐标下小车当前位置
    double car2goal_x = odom_goal_pos.x - car_pose.x;         //小车的目标位置与当前位置的差
...
//回调函数 定时调用 检测是否到达目标位置
void L1Controller::goalReachingCB(const ros::TimerEvent&)   
{

    if(goal_received)
    {
        double car2goal_dist = getCar2GoalDist();     //获取距离目标位置的距离
        if(car2goal_dist < goalRadius)                  //若小于拟定的衡量   标准
        {
            goal_reached = true;                        //目标位置  已达到
            goal_received = false;                      //下一个目标 还未接受
            ROS_INFO("Goal Reached !");
        }
    }
}
...
void L1Controller::controlLoopCB(const ros::TimerEvent&)  
{
    ...
    geometry_msgs::Pose carPose = odom.pose.pose;           //获取小车当前位置
    geometry_msgs::Twist carVel = odom.twist.twist;         //获取小车当前姿态
    cmd_vel.linear.x = 1500;                                //初始化线速度
    cmd_vel.angular.z = baseAngle;                          //初始化舵机角度

    if(goal_received)               
    {
        /*Estimate Steering Angle*/
        double eta = getEta(carPose);  //传入odom下小车的位置信息
        if(foundForwardPt)    //小车还未到达
        {
