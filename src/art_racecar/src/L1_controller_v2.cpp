...
geometry_msgs::Point L1Controller::get_odom_car2WayPtVec(const geometry_msgs::Pose& carPose)    
{
    ...
    if(!goal_reached)        //若目标位置没有达到
    {
        ...
        for(int i =0; i< map_path.poses.size(); i+=4)    //遍历得到的规划的路径信息
        {
            if(i >= map_path.poses.size())
            geometry_msgs::PoseStamped map_path_pose = map_path.poses[map_path.poses.size()];
            else
            ...
            try
            {
                tf_listener.transformPose("odom", ros::Time(0) , map_path_pose, "map" ,odom_path_pose);  //将map框架下得到的路径信息 转化 为odom坐标系下的路径信息
...
{
    double u = (Vcmd - current_v)*Gas_gain;             // (预计速度  - 当前速度 ) * kp;
